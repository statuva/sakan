...
function parseGenerateRequest(value) {
    ...
    if (!types_1.GENERATE_FEATURES.includes(feature)) {
        invalid("Unknown AI feature.");
    }
    const grounding = data.grounding == null ? {} : objectValue(data.grounding, "grounding");
...
function validateHomeInsightIds(grounding) {
    for (const key of [
        "relatedMomentId",
        "relatedInstanceId",
        "relatedReminderId",
...
function parseChatRequest(value) {
    ...
    return {
        ...
        recentMessages,
        timezoneOffsetMinutes: boundedInteger(data.timezoneOffsetMinutes, "timezone offset", -840, 840),
        locale: parseLocale(data.locale),
    };
}
...
function rejectUnknownKeys(data, allowed) {
...
function assertSafeJson(value, depth) {
    if (depth > 7)
        invalid("The grounding payload is too deeply nested.");
    if (value == null ||
        typeof value === "string" ||
    ...
    if (Array.isArray(value)) {
        if (value.length > 80)
            invalid("A grounding list is too long.");
    ...
    if (typeof value === "object") {
        ...
        if (entries.length > 80)
            invalid("A grounding object is too large.");
        for (const [key, item] of entries) {
            if (key.length > 80)
                invalid("A grounding field name is too long.");
